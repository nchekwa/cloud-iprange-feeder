...
def list_prefixes(ipv4=True):
    ...
    pfx_dict = {}
    for region_item in ipranges['regions']:
        for cidr_item in region_item['cidrs']:
            prefix = cidr_item['cidr']
            ...
            if ip_prefix not in pfx_dict:
                ...
                pfx_dict[ip_prefix]['rgn'] = [ region_item['region'] ]
                pfx_dict[ip_prefix]['svc'] = list()
                for tag in cidr_item['tags']:
                    pfx_dict[ip_prefix]['svc'].append(tag)
                    pfx_dict[ip_prefix]['svc'].append(tag+"_"+region_item['region'])
...
if __name__ == "__main__":
    ...
    if len(ipranges['regions'][0]['cidrs']) < 1:
